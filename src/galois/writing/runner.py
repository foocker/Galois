...
def _build_prompts(
    *,
    ...
    artifact_contract = (
        ...
        "If authors are provided, manuscript_draft.md must include them below the title; if authors are missing, write `Author not provided` below the title and create a revision task. "
        "Every used reference must appear in manuscript_draft.md as an inline citation using either `\\cite{key}` for keyed BibTeX-style sources or `[n]` for numbered references. "
        "Do not list a source as used merely because it appears in the bibliography. References not cited in the manuscript must be placed under an unused-reference audit. "
        ...
        "`Authors`, continuation feedback, and previous artifacts are materials; use any provided writing parameters for reference-count targets, page-count targets, and self-review revision rounds. "
        "Make `citation_report.md` begin with `# References`, then include `## Used References`, `## Unused References`, and `## Lookup Needed` sections. "
        "`Used References` may contain only sources cited inline in manuscript_draft.md. "
