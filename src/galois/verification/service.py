"""Galois-owned pure-language proof verification service.

This is the internalized implementation of the Rethlas verification API. The
vendored ``verification/api/server.py`` file remains as a compatibility
...
WORK_DIR = Path(os.getenv("GALOIS_VERIFICATION_WORKDIR", os.getenv("GALOIS_VERIFICATION_AGENT_DIR", str(ASSET_DIR)))).resolve()
RUNTIME_DIR = Path(os.getenv("GALOIS_VERIFICATION_RUNTIME_DIR", str(WORK_DIR))).resolve()
...
def _verification_path(run_id: str) -> Path | None:
...
def build_prompt(run_id: str, statement: str, proof: str) -> str:
    return (
        ...
        "Do not read local SKILL.md files unless you are blocked on a concrete ambiguity in AGENTS.md. "
        "If the proof cites no external paper theorem, skip external-reference search. "
        "Once you have enough evidence, write the verification output immediately."
    )
...
def build_codex_command(run_id: str, statement: str, proof: str) -> list[str]:
    return [
        ...
        CODEX_MODEL,
        "--config",
        f"model_reasoning_effort={normalize_reasoning_effort(CODEX_REASONING_EFFORT)}",
