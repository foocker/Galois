...
### Open Problem Research Protocol

When the problem appears to be an open conjecture or open problem, enter Open Problem Research Mode. In this mode, openness is background context only; it must not become a final answer, a stopping reason, or a reason to produce only a literature survey.

Every open-problem iteration must perform the following protocol before any final response is considered.

1. Frontier selection.
   Query `memory_search` and read the relevant local `memory/{problem_id}/` records for `failed_paths`, `proof_steps`, `subgoals`, `counterexamples`, and `branch_states`. Select exactly one frontier target for the next research move, such as a residual family, a missing inequality, a structural reduction, a toy model, a computational experiment, or a candidate lemma.

2. Novelty gate.
   Before acting, state why the selected frontier is not merely repeating a previous failed path. If it reuses a previous plan, identify the new mathematical question, new parameter range, new lemma, sharper bound, or different proof mechanism being tested. If no novelty can be stated, do not continue that route; choose a different frontier.

3. Frontier record.
   Immediately persist a `branch_update` record whose state includes:
   - `mode`: `open_problem_research`
   - `frontier_target`
   - `novelty_claim`
   - `expected_concrete_artifact`
   - `fallback_if_failed`
   - `next_exact_action`

4. Concrete research action.
   Each iteration must produce at least one concrete mathematical artifact, not just prose about known results. Acceptable artifacts include:
   - a proved lemma or conditional lemma;
   - a derived inequality or bound with explicit hypotheses;
   - a worked residual-family analysis;
   - a finite-field computation or toy example with interpretation;
   - a counterexample to a proposed reduction;
   - a killed plan together with a strictly new next-generation plan.

5. Stagnation rule.
   If two consecutive iterations end with the same failure reason, do not continue the same method. Switch level or method: from global theorem to residual family, from residual family to small examples, from examples to a conjectured lemma, from a conjectured lemma to proof/counterexample search, or from search to independent reasoning.

6. Continuation checkpoint.
   If no full proof has been assembled, do not write a final answer or closing summary. Instead persist a latest frontier checkpoint via `branch_update` and `memory_append` containing:
   - current frontier target;
   - last nontrivial result;
   - exact obstruction;
   - next exact action;
   - which skill should be used next.
   Also maintain `results/{problem_id}/blueprint.md` as a living research ledger. This file is not a final answer until verified; it must record the current statement, solved or partially solved subcases, failed frontiers with evidence, reusable experiment paths, and the next research checkpoint. Update it after any substantial frontier result so progress is not trapped only in logs or memory.

7. Resume behavior.
   On resume, first locate the latest frontier checkpoint in `memory/{problem_id}/branch_states.jsonl` and continue from its `next exact action` before doing any new broad search or summary. Do not repeat downloads, source summaries, or failed proof routes unless the Novelty gate is satisfied.

8. No final answer without verification.
   Do not write a final answer, final status report, or "honest conclusion" merely because the problem is open or because the current frontier failed. The only acceptable final output is after `blueprint_verified.md` has been produced. If the current frontier fails, record the failure and immediately select the next frontier.
   This restriction does not forbid writing a research checkpoint into `results/{problem_id}/blueprint.md`; it forbids presenting that checkpoint as a completed solution.


### Computational Experiment Persistence

One-off sanity checks may remain only in the run log. A temporary heredoc is acceptable for quick arithmetic checks, command probing, or throwaway calculations that do not support a proof step, branch decision, or future frontier.

Promote an experiment from temporary to persistent when any of the following is true:

- its output is used to justify a `proof_steps`, `failed_paths`, `counterexamples`, `subgoals`, or `branch_states` record;
- it supports a claim in `results/{problem_id}/blueprint.md`;
- it will likely be rerun with different parameters;
- it encodes nontrivial formulas, finite-field computations, sieves, searches, or residual-family analysis;
- it would be costly or error-prone to reconstruct from the log.

For promoted experiments:

1. Save reusable code under `experiments/{problem_id}/` with a descriptive filename.
2. Save structured output next to the code as JSON, JSONL, CSV, or markdown tables.
3. Add a short README or header comment explaining the mathematical question, inputs, outputs, and how to rerun it.
4. Append a `memory_append` record pointing to the code path, data path, command used, and the conclusion supported.
5. If an experiment supersedes a previous one, keep the old artifact but mark the newer one as preferred in the README or memory record.
...
Before downloading a paper, inspect `downloads/{problem_id}/` and any local README or manifest in that directory. Reuse existing PDFs, source archives, extracted TeX, and extracted text when they match the needed source. Do not create generic `latest/` download locations for reusable sources; prefer stable paper-specific names and record the provenance when adding new files.
