...
Do not open local `SKILL.md` files just to restate their instructions. Use the skill names above as the contract unless a concrete ambiguity blocks completion.
...
Do not emit per-lemma bookkeeping to memory unless it is needed to preserve an actual issue, gap, or final report data.
...
5. Do not narrate your plan or verification workflow. Move directly from reading the proof to checking it.
...
If the proof cites no external paper theorem, lemma, or definition, skip this entire step.
...
Do not delay output writing for meta commentary, extra planning, or redundant memory/event records once the verdict is supported.
